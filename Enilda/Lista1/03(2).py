...
valor_total= 0
pedido = int(input("Pronto para fazer o seu pedido?\nDigite 1 para sim. \nDigite 2 para não."))

while pedido == 1:
    cod = int(input("Informe o número(cód) do seu pedido: "))
    qnt = int(input("Agora nos informe a quantidade: "))    
    match cod:
        case 1:
            preco = 18
            parcial=preco*qnt
            valor_total= parcial + valor_total
            print(f"Seu pedido é {qnt} marmitex mini no valor de R${parcial:.2f}")
        case 2:
            preco = 20
            parcial=preco*qnt
            valor_total= parcial + valor_total
            print(f"Seu pedido é {qnt} marmitex pequeno no valor de R${parcial:.2f}.")
        case 3:
            preco = 23
            parcial=preco*qnt
            valor_total= parcial + valor_total
            print(f"Seu pedido é {qnt} marmitex médio no valor de R${parcial:.2f}.")
        case 4:
            preco = 26
            parcial=preco*qnt
            valor_total= parcial + valor_total
            print(f"Seu pedido é {qnt} marmitex grande no valor de R${parcial:.2f}.")
        case 5:
            preco = 30
            parcial = preco*qnt
            valor_total= parcial + valor_total
            print(f"Seu pedido é {qnt} marmitex fámilia no valor de R${parcial:.2f}.")

    pedido = int(input("Gostaria de acrescentar mais pedidos?.\nDigite 1 para sim. \nDigite 2 para não."))
gorjeta=input(f"O valor total do seu pedido é R${valor_total:.2f}.\nGostaria de contribuir com gorjeta de 10%?\nDigite S para sim e N para não:\n")
if gorjeta == "S":
    valor = (valor_total*0.10)
    total = valor+valor_total
    print(f"O valor total de sua compra ficou em R${total:.2f}. \nObrigado pela atenção e volte sempre.")
else:
    print(f"O valor total de sua compra ficou em R${valor_total:.2f}.\nObrigado pela atenção e volte sempre.")
